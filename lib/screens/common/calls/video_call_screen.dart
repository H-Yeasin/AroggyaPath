...
class _VideoCallScreenState extends State<VideoCallScreen> {
  ...
  Future<void> _initializeCall() async {
    ...
    try {
      ...
      await ActiveCallState.saveActiveCall(
        chatId: widget.chatId, userName: widget.userName,
        userAvatar: widget.userAvatar, otherUserId: widget.otherUserId,
        isInitiator: widget.isInitiator, callType: 'video',
      );

      await _agoraService.initialize();
      _agoraService.onUserJoined = (uid, elapsed) {
        if (mounted) {
          setState(() { _remoteUid = uid; _isCallConnected = true; _callStatus = 'Connected'; });
          _startCallTimer();
        }
      };
      _agoraService.onUserOffline = (uid, reason) {
        if (mounted) setState(() { _remoteUid = null; _callStatus = 'User Offline'; });
      };

      if (_agoraService.remoteUids.isNotEmpty) {
        final existingUid = _agoraService.remoteUids.first;
        if (mounted) { setState(() { _remoteUid = existingUid; _isCallConnected = true; _callStatus = 'Connected'; }); _startCallTimer(); }
      }
    ...
    try {
      ...
      if (token == null) {
        for (int attempt = 0; attempt < 2; attempt++) {
          try {
            final result = await ApiService.getAgoraToken(channelName: widget.chatId).timeout(const Duration(seconds: 8));
      ...
      if (_currentUserId != null) {
        await _agoraService.joinChannelWithUserAccount(channelName: widget.chatId, userAccount: _currentUserId!, isVideo: true, token: token);
      } else {
        await _agoraService.joinChannel(channelName: widget.chatId, uid: 0, isVideo: true, token: token);
      }

      if (mounted) { setState(() { _callStatus = 'Connected'; _isCallConnected = true; }); _startCallTimer(); }
    } catch (e) {
  ...
  void _setupSocketListeners() {
    ...
    socket.off('call:accepted'); socket.off('call:accept');
    ...
    void handleCallAccepted(dynamic data) async {
      if (data['chatId'] == widget.chatId && !_channelJoined) {
        _unansweredTimer?.cancel(); _unansweredTimer = null;
  ...
  void _toggleVideo() { setState(() => _isVideoOff = !_isVideoOff); _agoraService.toggleVideo(_isVideoOff); }
  void _switchCamera() { _agoraService.switchCamera(); }

  void _endCall() async {
    if (_isDisposed) return;
    _isDisposed = true; _callTimer?.cancel();
    ...
    try {
      if (widget.otherUserId.isNotEmpty && widget.isInitiator) {
        await AgoraChatService.instance.sendCallLog(
          conversationId: widget.otherUserId, callType: 'video',
  ...
  void dispose() {
    ...
    _callTimer?.cancel(); _unansweredTimer?.cancel();
    if (!_isDisposed) {
      _isDisposed = true;
              ...
              child: Center(
                child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                  CircleAvatar(
                    ...
                    backgroundImage: widget.userAvatar != null && widget.userAvatar!.isNotEmpty ? NetworkImage(widget.userAvatar!) : null,
                    child: widget.userAvatar == null ? const Icon(Icons.person, color: Colors.white70, size: 60) : null,
                  ),
                  ...
                  if (_isInitializing || _callStatus == 'Calling...' || _callStatus == 'Connecting...')
            ...
            Positioned(
              ...
              child: Container(
                ...
                child: ClipRRect(
                  ...
                  child: AgoraVideoView(
                    controller: VideoViewController(rtcEngine: _agoraService.engine!, canvas: const VideoCanvas(uid: 0)),
                  ),
                ),
              ),
            ),
            ...
            child: SafeArea(
              child: Container(
                ...
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
                  _buildBtn(_isMuted ? Icons.mic_off : Icons.mic, _isMuted ? 'Unmute' : 'Mute', _toggleMute, _isMuted ? Colors.red : Colors.white),
                  _buildBtn(_isVideoOff ? Icons.videocam_off : Icons.videocam, _isVideoOff ? 'Cam Off' : 'Cam On', _toggleVideo, _isVideoOff ? Colors.red : Colors.white),
                  _buildBtn(Icons.cameraswitch, 'Switch', _switchCamera, Colors.white),
                  _buildEndBtn(),
                ]),
              ),
            ),
  ...
  Widget _buildBtn(IconData icon, String label, VoidCallback onTap, Color color) {
    return Column(mainAxisSize: MainAxisSize.min, children: [
      GestureDetector(onTap: onTap, child: Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.white.withOpacity(0.2), shape: BoxShape.circle), child: Icon(icon, color: color, size: 28))),
  ...
  Widget _buildEndBtn() {
    return Column(mainAxisSize: MainAxisSize.min, children: [
      GestureDetector(onTap: _endCall, child: Container(padding: const EdgeInsets.all(16), decoration: const BoxDecoration(color: Colors.red, shape: BoxShape.circle), child: const Icon(Icons.call_end, color: Colors.white, size: 28))),
      const SizedBox(height: 8), const Text('End', style: TextStyle(color: Colors.white, fontSize: 12)),
    ]);
  }
}
